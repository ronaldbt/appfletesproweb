/**
 * Genera pages/fletes-{slug}.vue para cada comuna (excepto santiago, ya existe).
 * Ejecutar: node scripts/generate-comuna-pages.cjs
 */
const fs = require('fs')
...
const comunasRM = [
  'cerrillos', 'cerro-navia', 'conchali', 'el-bosque', 'estacion-central', 'huechuraba', 'independencia',
  'la-cisterna', 'la-florida', 'la-granja', 'la-pintana', 'la-reina', 'las-condes', 'lo-barnechea', 'lo-espejo', 'lo-prado',
  'macul', 'maipu', 'nunoa', 'pedro-aguirre-cerda', 'penalolen', 'providencia', 'pudahuel', 'quilicura', 'quinta-normal',
  'recoleta', 'renca', 'san-joaquin', 'san-miguel', 'san-ramon', 'vitacura', 'pirque', 'puente-alto', 'san-jose-de-maipo',
  'colina', 'lampa', 'tiltil', 'buin', 'calera-de-tango', 'paine', 'san-bernardo', 'alhue', 'curacavi', 'maria-pinto', 'san-pedro',
  'el-monte', 'isla-de-maipo', 'padre-hurtado', 'talagante'
]

const template = (slug) => `<template>
    ...
    <section class="py-24 bg-white">
      <div class="container mx-auto px-4">
        <div class="max-w-3xl mx-auto">
          ...
          <p class="text-slate-600 leading-relaxed mb-10 font-medium">¿Qué nos distingue? Razones para elegir nuestro servicio de fletes y mudanzas en la comuna:</p>
...
<script setup>
import content from '~/data/comunas/${slug}.js'
...
useHead({
  ...
  meta: [
    ...
    { property: 'og:url', content: \`\${siteUrl}/fletes-\${routeSlug}\` },
  ...
  link: [{ rel: 'canonical', href: \`\${siteUrl}/fletes-\${routeSlug}\` }]
})
...
comunasRM.forEach(slug => {
  const outPath = path.join(pagesDir, `fletes-${slug}.vue`)
  fs.writeFileSync(outPath, template(slug), 'utf8')
  console.log('OK', `fletes-${slug}.vue`)
})
console.log('Total:', comunasRM.length)
