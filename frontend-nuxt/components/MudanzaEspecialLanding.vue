...
const props = defineProps({
  ...
  /** Ruta canónica sin dominio, ej. /fletes-piano */
  canonicalPath: {
    type: String,
    required: true
  }
...
const tp = (suffix) => t(`pages.${props.pageKey}.${suffix}`)
...
const services = computed(() => [
  {
    icon: IconHome,
    title: tp('services.service1.title'),
    desc: tp('services.service1.description')
  },
  {
    icon: IconPackage,
    title: tp('services.service2.title'),
    desc: tp('services.service2.description')
  },
  {
    icon: IconShield,
    title: tp('services.service3.title'),
    desc: tp('services.service3.description')
  }
])

const benefits = computed(() => [
  { icon: '💰', title: tp('benefits.benefit1.title'), desc: tp('benefits.benefit1.description') },
  { icon: '⚡', title: tp('benefits.benefit2.title'), desc: tp('benefits.benefit2.description') },
  { icon: '🛡️', title: tp('benefits.benefit3.title'), desc: tp('benefits.benefit3.description') },
  { icon: '📦', title: tp('benefits.benefit4.title'), desc: tp('benefits.benefit4.description') }
])

const processSteps = computed(() => [
  { title: tp('process.step1.title'), description: tp('process.step1.description') },
  { title: tp('process.step2.title'), description: tp('process.step2.description') },
  { title: tp('process.step3.title'), description: tp('process.step3.description') },
  { title: tp('process.step4.title'), description: tp('process.step4.description') }
])

const faqCount = 6
const faqOpenStates = ref(Array(faqCount).fill(false))
...
const faqs = computed(() => {
  ...
  const base = `pages.${props.pageKey}.faqs`
  return Array.from({ length: faqCount }, (_, i) => ({
...
useHead(computed(() => ({
  title: tp('seo.title'),
  meta: [
    { name: 'description', content: tp('seo.description') },
    { property: 'og:type', content: 'website' },
    { property: 'og:url', content: currentUrl.value },
    { property: 'og:title', content: tp('seo.ogTitle') },
    { property: 'og:description', content: tp('seo.ogDescription') },
    { property: 'og:image', content: defaultImage },
    { property: 'og:site_name', content: 'FletesPro' },
    { property: 'og:locale', content: 'es_ES' },
    { name: 'twitter:card', content: 'summary_large_image' },
    { name: 'twitter:url', content: currentUrl.value },
    { name: 'twitter:title', content: tp('seo.twitterTitle') },
    { name: 'twitter:description', content: tp('seo.twitterDescription') },
  ...
  script: [
    {
      ...
      children: JSON.stringify({
        ...
        '@type': 'Service',
        serviceType: tp('seo.schemaServiceType'),
        provider: {
          '@type': 'LocalBusiness',
          name: 'FletesPro',
          url: siteUrl
        ...
        description: tp('seo.schemaDescription')
      })
    },
    {
      type: 'application/ld+json',
