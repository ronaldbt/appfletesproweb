<template>
  <FletesComunaLayout :hero="content.hero" variant="las-condes">
    <!-- Precios y valores (H2) -->
    ...
    <!-- Servicios: camioneta, furgón, mini (H2) -->
      ...
      <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div class="space-y-6">
          <article
            ...
            :key="i"
            class="bg-slate-50 rounded-r-2xl border-l-4 border-amber-500 p-6 pl-6"
          >
            <h3 class="text-xl font-black text-slate-900 mb-2">{{ item.h3 }}</h3>
            <p class="text-slate-600 leading-relaxed">{{ item.text }}</p>
          </article>
        </div>
        <p class="mt-6 text-slate-600 leading-relaxed">{{ content.servicios.pInmediatos }}</p>
      </div>
    </section>

    <!-- Fletes y mudanzas en la comuna (H2) -->
    <section class="py-16 md:py-20 bg-slate-50">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-4 pb-3 border-b-4 border-amber-500 w-fit">
          {{ content.mudanzas.h2 }}
    ...
    <section class="py-16 md:py-20 bg-white">
      <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div class="mt-10 flex flex-wrap gap-4">
          ...
          <button type="button" @click="scrollToCalc" class="inline-flex items-center gap-2 border-2 border-amber-600 text-amber-700 px-6 py-4 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-amber-50 transition-colors">Cotizar con la calculadora</button>
        </div>
      </div>
    </section>

    <!-- FAQs (H2) -->
    <section class="py-16 md:py-20 bg-slate-50" id="faqs">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-6 pb-3 border-b-4 border-amber-500 w-fit">
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 2, name: 'Fletes Las Condes', item: currentUrl }
  ]
}
...
const serviceSchema = {
  '@context': 'https://schema.org',
  '@type': 'Service',
  serviceType: 'Fletes y mudanzas en Las Condes',
