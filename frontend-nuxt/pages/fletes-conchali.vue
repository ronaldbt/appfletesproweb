<template>
  <FletesComunaLayout :hero="content.hero" variant="conchali">
    <section class="py-16 md:py-20 bg-white border-t border-slate-100">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-4 pb-3 border-b-4 border-rose-500 w-fit">{{ content.precios.h2 }}</h2>
    ...
    <section class="py-16 md:py-20 bg-rose-50/30">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-4 pb-3 border-b-4 border-rose-500 w-fit">{{ content.rutas.h2 }}</h2>
    ...
    <section class="py-16 md:py-20 bg-white">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-4 pb-3 border-b-4 border-rose-500 w-fit">{{ content.servicios.h2 }}</h2>
        <p class="text-slate-600 leading-relaxed mb-8">{{ content.servicios.intro }}</p>
        <div class="space-y-6">
          <article v-for="(item, i) in content.servicios.items" :key="i" class="bg-rose-50/50 rounded-r-2xl border-l-4 border-rose-500 p-6 pl-6">
    ...
    <section class="py-16 md:py-20 bg-rose-50/30">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-4 pb-3 border-b-4 border-rose-500 w-fit">{{ content.mudanzas.h2 }}</h2>
    ...
    <section class="py-16 md:py-20 bg-white">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-6 pb-3 border-b-4 border-rose-500 w-fit">{{ content.h2Ventajas }}</h2>
        <ol class="space-y-4 list-none pl-0">
          <li v-for="(v, i) in content.ventajas" :key="i" class="flex gap-4 text-slate-600 leading-relaxed">
            <span class="flex-shrink-0 w-10 h-10 rounded-xl bg-rose-500 text-white flex items-center justify-center font-black text-lg">{{ i + 1 }}</span>
            <span>{{ v }}</span>
          </li>
        </ol>
        <p class="mt-10 text-slate-600 leading-relaxed font-medium">{{ content.cierre }}</p>
        <div class="mt-10 flex flex-wrap gap-4">
          <a href="https://wa.me/56979796841" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 bg-rose-600 text-white px-6 py-4 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-rose-700 transition-colors">Contactar por WhatsApp</a>
          <button type="button" @click="scrollToCalc" class="inline-flex items-center gap-2 border-2 border-rose-600 text-rose-700 px-6 py-4 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-rose-50 transition-colors">Cotizar con la calculadora</button>
        </div>
      </div>
    </section>
    <section class="py-16 md:py-20 bg-rose-50/30" id="faqs">
      <div class="container mx-auto px-4 max-w-4xl">
        <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-6 pb-3 border-b-4 border-rose-500 w-fit">Preguntas frecuentes: fletes Conchalí</h2>
        <ul class="space-y-6">
...
<script setup>
import content from '~/data/comunas/conchali.js'
const siteUrl = 'https://fletespro.cl'
const routeSlug = 'conchali'
